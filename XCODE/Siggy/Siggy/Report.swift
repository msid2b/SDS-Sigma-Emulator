...
class SystemStatusReport: PDFReport {
    ...
    override func main() {
        ...
        printHeading("CPU")
        if (cpu.fault) {
            printHeading("FAULT: " + cpu.faultMessage)
            printHeading("AT: "+MSDate(gmtTimestamp: cpu.faultTime).ISO8601Format(timeZone: MSDate.GMT) )
        }
        ...
        tsa("LAST WAIT INSTrUCTION: "+MSDate(gmtTimestamp: cpu.waitInstructionTime).ISO8601Format(timeZone: MSDate.GMT))
        tsa("LAST WAIT SIGNAL: "+MSDate(gmtTimestamp: cpu.waitSignalTime).ISO8601Format(timeZone: MSDate.GMT))
        tsa("REGISTERS:")
        ...
        printHeading("WAIT TRACE", threshold: 0.8)
        tsaTraceHeading()
        var i = 0
        var e = cpu.waitTrace?.entry(i)
        while (e != nil ) {
            tsaTrace(e!)
            i += 1
            e = cpu.waitTrace?.entry(i)
        }
        tsa("")
        ...
        if (cpu.interrupts.activeInterruptData.first(where: { (s) -> Bool in return (s != nil) }) != nil) {
            tsa("ACTIVE INTERRUPTS")
            var x = 0
            while (x < cpu.interrupts.activeInterruptData.count) {
                if let id = cpu.interrupts.activeInterruptData[x] {
                    tsaInterrupt(id)
                }
                x += 1
            }
            ...
            tsa("NO ACTIVE INTERRUPTS")
        }
        if (iss.waiting.isEmpty) {
            tsa("NO WAITING INTERRUPTS")
        }
        else {
            var id: InterruptSubsystem.InterruptData?
            let count = iss.waiting.count
            tsa("\(count) WAITING INTERRUPTS")
            id = iss.waiting.firstObject() as? InterruptSubsystem.InterruptData
            var x = count
            while (id != nil) && (x > 0) {
                tsaInterrupt(id!)
            ...
            if (id != nil) {
                tsaRed("WAITING INTERRUPT LIST BROKEN...")
            }
        }
        tsa("")
